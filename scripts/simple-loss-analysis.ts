...
class SimpleLossAnalysis {
    ...
    private async getLosingTrades(): Promise<any[]> {
                    ...
                    if (signal.action !== 'HOLD') {
                        ...
                        if (confidence >= 0.75) { // Use conservative strategy
                            ...
                            if (!tradeResult.success) {
                                ...
                                const timeOfDay = current.timestamp.toTimeString().substring(0, 5);
    ...
    private analyzePatterns(losingTrades: any[]): any[] {
    ...
    private addPattern(patterns: { [key: string]: any }, name: string, trade: any): void {
        if (!patterns[name]) {
    ...
    private displayLossAnalysis(patterns: any[], losingTrades: any[]): void {
    ...
    private generateFilters(patterns: any[]): any {
        const filters: any = {
            rsi: { min: 40, max: 60 },
    ...
    private async saveAnalysis(patterns: any[], filters: any): Promise<void> {
        try {
...
// Main execution
async function main() {
    const analysis = new SimpleLossAnalysis();
