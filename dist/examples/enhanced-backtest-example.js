...
exports.runEnhancedBacktestExample = runEnhancedBacktestExample;
exports.demonstrateLiveTradeRecommendations = demonstrateLiveTradeRecommendations;
exports.getPerformanceGrade = getPerformanceGrade;
const enhanced_backtest_service_1 = require("../services/enhanced-backtest.service");
const logger_1 = require("../logger/logger");
async function runEnhancedBacktestExample() {
    try {
        logger_1.logger.info('Starting Enhanced Backtest Example');
        const backtestService = new enhanced_backtest_service_1.EnhancedBacktestService();
        ...
        logger_1.logger.info('Running enhanced backtest with configuration:', backtestConfig);
        ...
        logger_1.logger.info('Saving backtest results to database...');
        ...
        const liveRecommendations = await backtestService.getTradeRecommendations(backtestConfig.symbols, '1day');
        ...
        logger_1.logger.info('Enhanced backtest example completed successfully');
    }
    catch (error) {
        logger_1.logger.error('Enhanced backtest example failed:', error);
        throw error;
    }
}
function getPerformanceGrade(results) {
    let score = 0;
    if (results.annualizedReturn > 0.20)
        score += 30;
    else if (results.annualizedReturn > 0.15)
        score += 25;
    else if (results.annualizedReturn > 0.10)
        score += 20;
    else if (results.annualizedReturn > 0.05)
        score += 10;
    if (results.sharpeRatio > 2.0)
        score += 25;
    else if (results.sharpeRatio > 1.5)
        score += 20;
    ...
    if (results.winRate > 0.7)
        score += 20;
    else if (results.winRate > 0.6)
        score += 16;
    else if (results.winRate > 0.5)
        score += 12;
    else if (results.winRate > 0.4)
        score += 8;
    if (results.maxDrawdown < 0.05)
        score += 15;
    else if (results.maxDrawdown < 0.10)
        score += 12;
    else if (results.maxDrawdown < 0.15)
        score += 8;
    else if (results.maxDrawdown < 0.20)
        score += 4;
    if (results.profitFactor > 2.0)
        ...
        score += 6;
    else if (results.profitFactor > 1.0)
        score += 3;
    if (score >= 85)
        return 'A+';
    if (score >= 80)
...
async function demonstrateLiveTradeRecommendations() {
    try {
        logger_1.logger.info('Demonstrating live trade recommendations');
        const backtestService = new enhanced_backtest_service_1.EnhancedBacktestService();
        ...
        logger_1.logger.error('Failed to demonstrate live trade recommendations:', error);
        throw error;
    }
}
async function main() {
    try {
        ...
        logger_1.logger.error('Example execution failed:', error);
        process.exit(1);
    }
}
...
//# sourceMappingURL=enhanced-backtest-example.js.map