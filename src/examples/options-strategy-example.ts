...
export class OptionsStrategyExample {
    async runOptionsStrategyExample(): Promise<void> {
        try {
            logger.info('Starting Options Strategy Example');

            // 1. Create options strategies from templates
            await this.createOptionsStrategiesFromTemplates();

            // 2. Create custom options strategies
            await this.createCustomOptionsStrategies();

            // 3. Demonstrate different options strategies
            await this.demonstrateOptionsStrategies();

            // 4. Options risk management
            await this.optionsRiskManagement();

            logger.info('Options Strategy Example completed successfully');
        } catch (error) {
            logger.error('Options Strategy Example failed:', error);
        }
    }

    private async createOptionsStrategiesFromTemplates(): Promise<void> {
        logger.info('\n=== Creating Options Strategies from Templates ===');

        // 1. Covered Call Strategy
        const coveredCallStrategy = await enhancedStrategyService.createStrategyFromTemplate(
        ...
        logger.info(`Created Covered Call Strategy: ${coveredCallStrategy.name}`);

        // 2. Iron Condor Strategy
        const ironCondorStrategy = await enhancedStrategyService.createStrategyFromTemplate(
        ...
        logger.info(`Created Iron Condor Strategy: ${ironCondorStrategy.name}`);

        // 3. Straddle Strategy
        const straddleStrategy = await enhancedStrategyService.createStrategyFromTemplate(
            'straddle_strategy',
            {
        ...
        logger.info(`Created Straddle Strategy: ${straddleStrategy.name}`);
    }

    private async createCustomOptionsStrategies(): Promise<void> {
        logger.info('\n=== Creating Custom Options Strategies ===');

        // 1. Custom Butterfly Spread Strategy
        const butterflyStrategy = await this.createCustomButterflyStrategy();
        logger.info(`Created Custom Butterfly Strategy: ${butterflyStrategy.name}`);

        // 2. Custom Calendar Spread Strategy
        const calendarStrategy = await this.createCustomCalendarStrategy();
        logger.info(`Created Custom Calendar Strategy: ${calendarStrategy.name}`);

        // 3. Custom Protective Put Strategy
        const protectivePutStrategy = await this.createCustomProtectivePutStrategy();
        logger.info(`Created Custom Protective Put Strategy: ${protectivePutStrategy.name}`);
    }

    private async createCustomButterflyStrategy(): Promise<any> {
        const butterflyConfig: StrategyConfig = {
        ...
        return await enhancedStrategyService.createStrategy(butterflyConfig);
    }

    private async createCustomCalendarStrategy(): Promise<any> {
        const calendarConfig: StrategyConfig = {
        ...
        return await enhancedStrategyService.createStrategy(calendarConfig);
    }

    private async createCustomProtectivePutStrategy(): Promise<any> {
        const protectivePutConfig: StrategyConfig = {
        ...
        return await enhancedStrategyService.createStrategy(protectivePutConfig);
    }

    private async demonstrateOptionsStrategies(): Promise<void> {
        ...
        for (const strategy of strategies) {
            ...
            logger.info(`Risk Profile: ${strategy.riskProfile}`);
            logger.info(`Best For: ${strategy.bestFor}`);
        }
    }

    private async optionsRiskManagement(): Promise<void> {
        ...
        for (const rule of riskManagementRules) {
            ...
            logger.info(`Implementation: ${rule.implementation}`);
        }

        // Demonstrate options-specific risk calculations
        ...
        for (const calc of riskCalculations) {
            logger.info(`\nMetric: ${calc.metric}`);
            logger.info(`Description: ${calc.description}`);
            logger.info(`Calculation: ${calc.calculation}`);
            logger.info(`Limit: ${calc.limit}`);
        }
    }
}
...
export default OptionsStrategyExample; 